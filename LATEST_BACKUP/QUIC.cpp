...
extern "C" void QUIC(char mode, uint32_t &p, const double *S, double *Lambda0,
					 ...
					 double *neg_logdetX_trSX)
{
	...
	for (; NewtonIter <= maxNewtonIter; NewtonIter++)
	{
		...
		// Save currnet value of Negative Log Likelihood and number of non-zero elements in X
		neg_logdetX_trSX[0] = -logdetX + trSX;
...
extern "C" void QUICR(char **modePtr, uint32_t &p, const double *S, double *Lambda0,
					  uint32_t &pathLen, const double *path, double &tol,
					  int32_t &msg, uint32_t &maxIter,
					  double *X, double *W, double *opt, double *cputime,
					  uint32_t *iter, double *dGap, double *neg_logdetX_trSX)
{
	...
	QUIC(mode, p, S, Lambda0, pathLen, path, tol, msg, maxIter, X, W,
		 opt, cputime, iter, dGap, neg_logdetX_trSX);
}
